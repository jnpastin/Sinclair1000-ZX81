...
void helper_ld_XY_nn(char targetRegister, char targetRegister2, byte* data)
{
  ...
  byte localData[]={inst.opCode, data[0], data[1]};
  uint16_t addr[]={0x00};
  writeSingleInstruction(localData, 1, 2, 0, addr);
}
...
void helper_ld_ptr_nn_hl(byte* data)
{
  //Expects a byte array consisting of:
  // [0] = Address Low Byte
  // [1] = Address High Byte
  ...
  char targetMnemonic[]="LD (nn),HL";
  int instIndex=findInstructionIndexByMnemonic(targetMnemonic);
  instructionDefinitionType inst;
  memcpy_P(&inst, &InstructionDefinitions[instIndex], sizeof(inst));

  uint16_t addr[]={0x00};
  byte localData[]={inst.opCode, data[0], data[1], data[2], data[3]};
  writeSingleInstruction(localData, 1, 2, 2, addr);
